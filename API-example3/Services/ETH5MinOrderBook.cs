...
{
    class ETH5MinOrderBook : IETH5MinOderBook
    {
        IAmazonDynamoDB amazonDynamoDB = new AmazonDynamoDBClient();

        public ETH5MinOrderBook(IAmazonDynamoDB amazonDynamoDB)
        {
        ...
        public async Task putJson(string key, string bitmexjson, string deribitjson)
        {
            try
            {
                Table table = Table.LoadTable(amazonDynamoDB, "ETH5MinOrderBook");
